...
def nodeAnalizer(solucion, matrix, nodesToInsert, objetivo, node, dimension, a, b):
    if (node[0]+a) not in {-1, dimension} and (node[1]+b) not in {-1, dimension} and solucion[node[0]+a][node[1]+b] == (-1,-1) and matrix[node[0]+a][node[1]+b] != "1":
...
def DFS(matrix, puntoInicial, objetivo, solucion):
    ...
    while stack:
        ...
        node = stack[0]
        stack.pop(0)
        nodeAnalizer(solucion, matrix, nodesToInsert, objetivo, node, dimension, -1, 0)
        nodeAnalizer(solucion, matrix, nodesToInsert, objetivo, node, dimension, 0, -1)
        nodeAnalizer(solucion, matrix, nodesToInsert, objetivo, node, dimension, 1, 0)
        nodeAnalizer(solucion, matrix, nodesToInsert, objetivo, node, dimension, 0, 1)
